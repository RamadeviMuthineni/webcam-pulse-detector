...
class findFaceGetPulse(object):
    ...
    def run(self, cam):
        ...
        if L > 10:
            ...
            if len(pruned) > 0:
                idx2 = np.argmax(pruned)
                t = (np.sin(phase[idx2]) + 1.) / 2.  # Initialize t from valid phase data
            else:
                print("No valid data in 'pruned'.")
                return  # Skip current iteration if no valid data
            # Smooth t value
            t = 0.9 * t + 0.1
            # Update frame with calculated alpha and beta
            alpha = t
            beta = 1 - t
            # Continue with the rest of the frame update logic
            # #t = 0.9 * t + 0.1
            #alpha = t
            #beta = 1 - t
